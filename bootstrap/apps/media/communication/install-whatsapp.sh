#!/usr/bin/env bash
# ~/.config/bootstrap/apps/media/communication/install-whatsapp.sh
# Purpose: Ensures whatsapp cask is installed via Homebrew and records its version.
# Usage: Run via bootstrap.sh or directly with: bash ~/.config/bootstrap/apps/media/communication/install-whatsapp.sh
set -euo pipefail
