#!/usr/bin/env bash
# ~/.config/bootstrap/apps/security/install-bitwarden.sh
# Purpose: Ensures bitwarden cask is installed via Homebrew and records its version.
# Usage: Run via bootstrap.sh or directly with: bash ~/.config/bootstrap/apps/security/install-bitwarden.sh
set -euo pipefail
