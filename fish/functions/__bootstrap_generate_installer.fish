...
# Purpose: Creates bootstrap/install-<pkg>.sh from template for a brew package.
...
function __bootstrap_generate_installer --description "Generate install-<pkg>.sh bootstrap script"
    ...
    set -l template "$root/bootstrap/install-template.sh"
    test -f "$template"; or return 1

    set -l safe_name (string lower -- "$pkg")
    set safe_name (string replace -ra '[^a-z0-9._-]' '-' -- "$safe_name")
    set -l target "$root/bootstrap/install-$safe_name.sh"

    # Respect existing custom scripts.
    if test -f "$target"
        return 0
    end
